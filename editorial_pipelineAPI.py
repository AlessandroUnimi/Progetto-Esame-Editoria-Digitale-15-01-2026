...
def main():
    ...
    # LETTURA AUTOMATICA DELLA CARTELLA
    files = [f for f in os.listdir(SOURCE_DIR) if os.path.splitext(f)[1].lower() in ALLOWED_EXTENSIONS]
    ...
    for filename in sorted(files): # Sorted garantisce l'ordine alfabetico per la build finale
        src_path = os.path.join(SOURCE_DIR, filename)
        # Creiamo il nome output cambiando l'estensione in .md
        ...
        final_output = result if result else f"\n{clean_text}"
