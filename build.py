...
# Definizione delle cartelle relative alla root dello script
# Usa os.path.join per garantire compatibilità tra Windows e Mac/Linux
...
# LISTA FILE ORDINATA
# Qui definiamo l'ordine logico di lettura, indipendentemente dal nome del file.
# Lo script cercherà questi file dentro la cartella "Content/Translated"
MD_FILES = [
    "00_Intro.md",              # Introduzione
    "01_text_generation.md",      # Modulo 1: Text Gen (Nonostante il nome file confuso)
    "02_language_detection.md", # Modulo 2: Embeddings
    "03_data_analysis.md"     # Modulo 3: Cross-Tabular (Nonostante il nome file confuso)
]

# Costruiamo i percorsi completi per pandoc
...
def run_pandoc(output_format, extension, extra_args=[]):
    ...
    try:
        ...
        # Decodifica dell'errore per renderlo leggibile
...
def main():
    ...
    epub_extra_args = [
        *epub_css_arg,
        ...
        "--metadata", "titlepage=false" # <--- Fondamentale
    ]
    
    run_pandoc("epub", "epub", epub_extra_args)

    # 3. PDF (Richiede LaTeX)
